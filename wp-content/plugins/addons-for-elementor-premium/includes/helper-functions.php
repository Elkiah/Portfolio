...
function lae_get_image_html($image_setting, $image_size_key, $settings) {
    ...
    if (!empty($attachment_id) && in_array($size, $image_sizes)) {
        ...
        if (!empty($image_src)) {
            ...
            $image_html .= sprintf('<img data-no-lazy="1" src="%s" title="%s" alt="%s"%s />', esc_attr($image_src), get_the_title($attachment_id), lae_get_image_alt($attachment_id), $image_class_html);
        }
    }
