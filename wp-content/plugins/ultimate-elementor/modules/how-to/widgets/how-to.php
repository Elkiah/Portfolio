	...
	protected function register_total_time_controls() {
		$this->start_controls_section(
			...
			$this->add_control(
				'show_time',
				array(
					'label'       => __( '<b>Show Total Time</b>', 'uael' ),
			...
			$this->add_control(
				'show_cost',
				array(
					'label'       => __( '<b>Show Estimated Cost</b>', 'uael' ),
					'type'        => Controls_Manager::SWITCHER,
					'label_on'    => __( 'Yes', 'uael' ),
					'label_off'   => __( 'No', 'uael' ),
					'default'     => 'yes',
	...
	protected function register_supply_controls() {
		$this->start_controls_section(
			...
			$this->add_control(
				'show_supply',
				array(
					'label'       => __( '<b>Add Materials</b>', 'uael' ),
					'type'        => Controls_Manager::SWITCHER,
					'label_on'    => __( 'Yes', 'uael' ),
					'label_off'   => __( 'No', 'uael' ),
					'default'     => 'yes',
		...
		if ( parent::is_internal_links() ) {
			...
			$this->add_control(
				...
				array(
					...
					'raw'             => sprintf( __( '%1$s Getting started article » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/how-to-schema/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
		...
		<div class="uael-howto-wrapper">
			...
			<?php if ( 'yes' === $settings['show_time'] || 'yes' === $settings['show_cost'] ) { ?>
				<div class="uael-howto-details">
					<?php if ( 'yes' === $settings['show_time'] ) { ?>
						<div class="uael-howto-time-needed">
							<span><span class="uael-howto-time-title"><?php echo wp_kses_post( $settings['time_text'] ) . ' '; ?></span><?php echo esc_html( $settings['time_needed'] ) . ' Minutes'; ?></span>
						</div>
					<?php } ?>
			...
			<script type="application/ld+json">
				...
				"step": 
					...
					?>
				<?php } ?>
				<?php if ( ! empty( $settings['time_needed'] ) && 'yes' === $settings['show_time'] ) { ?>
					, "totalTime": 
					<?php if ( isset( $settings['time_needed'] ) && ! empty( $settings['time_needed'] ) ) { ?>
						"PT<?php echo $settings['time_needed']; ?>M"
					<?php } ?>
				<?php } ?>
				}
			</script>
