 ...
 * Envato API class.
 *
 * @package BetterBuilder_Envato_API
 */
 ...
 *
 * @class BetterBuilder_Envato_API
 * @version 1.0.0
 * @since 1.0.0
 */
class BetterBuilder_Envato_API {
	 ...
	 * @access public
	 * @since 1.0.0
	 * @param object $token An Envato API token.
	 */
	...
	public function request( $url, $args = array() ) {
		...
		$defaults = array(
			'headers' => array(
				...
				'User-Agent' => 'WordPress - Better Builder Library',
			),
			'timeout' => 20,
		);
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', __( 'An API token is required.', 'better' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'better' ) );
		}

		$return = json_decode( wp_remote_retrieve_body( $response ), true );

		if ( null === $return ) {
			return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'better' ) );
		}
	...
	public function deferred_download( $id ) {
		...
		return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=betterbuilder' ) ) );
	}
	 ...
	 */
	public function username( $args = array() ) {
	...
	public function themes( $args = array(), $page = '' ) {
		...
		foreach ( $response['results'] as $theme ) {
			$item = $this->normalize_theme( $theme['item'] );
			$item['code'] = ( ! empty( $theme['code'] ) ? $theme['code'] : '' );	
			$themes[] = $item;
		}

		if ( 100 == count( $themes ) ) {
			$page = ( ! $page ) ? 2 : $page + 1;
			$themes = array_merge( $themes, $this->themes( $args, $page ) );
		}
	...
	public function plugins( $args = array(), $page = '' ) {
		...
		foreach ( $response['results'] as $plugin ) {
			$item = $this->normalize_plugin( $plugin['item'] );
			$item['code'] = ( ! empty( $plugin['code'] ) ? $plugin['code'] : '' );
			$plugins[] = $item;
		}

		if ( 100 == count( $plugins ) ) {
			$page = ( ! $page ) ? 2 : $page + 1;
			$plugins = array_merge( $plugins, $this->plugins( $args, $page ) );
		}
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'better' ), '1.0.0' );
	}
