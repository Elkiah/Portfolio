...
define( 'ELEMENTOR_EXTRAS_VERSION', 						'2.2.30' );
define( 'ELEMENTOR_EXTRAS_ELEMENTOR_VERSION_REQUIRED', 		'2.8.0' );
...
function elementor_extras_fail_load() {
	...
	$message = sprintf( __( 'You need %1$sElementor%2$s for %1$Extras%2$s to work.', 'elementor-extras' ), '<strong>', '</strong>' );
	...
	if ( _is_elementor_installed() ) {
		...
		$message = sprintf( __( 'You need to activate %1$sElementor%2$s for %1$Extras%2$s to work.', 'elementor-extras' ), '<strong>', '</strong>' );
...
function elementor_extras_fail_load_out_of_date() {
	$class = 'notice notice-error';
	$message = __( 'Extras requires at least Elementor version ' . ELEMENTOR_EXTRAS_ELEMENTOR_VERSION_REQUIRED . '. Please update Elementor and re-activate Extras.', 'elementor-extras' );
...
function elementor_pro_extras_fail_load_out_of_date() {
	$class = 'notice notice-error';
	$message = __( 'Extras requires you update Elementor Pro to at least version ' . ELEMENTOR_EXTRAS_ELEMENTOR_PRO_VERSION_REQUIRED . ' to avoid any issues. We\'ve deactivated Extras for now. Please update Elementor Pro and re-activate Extras.', 'elementor-extras' );
...
function elementor_extras_before_activation() {
	if ( ! version_compare( ELEMENTOR_VERSION, ELEMENTOR_EXTRAS_ELEMENTOR_VERSION_REQUIRED, '>=' ) ) {
		wp_die( __( 'Extras requires at least Elementor version ' . ELEMENTOR_EXTRAS_ELEMENTOR_VERSION_REQUIRED . '. Please update Elementor and re-activate Extras for Elementor.', 'elementor-extras' ) );
	}

	if ( is_elementor_pro_active() ) {
		if ( ! version_compare( ELEMENTOR_PRO_VERSION, ELEMENTOR_EXTRAS_ELEMENTOR_PRO_VERSION_REQUIRED, '>=' ) ) {
			wp_die( __( 'Extras requires you update Elementor Pro to at least version ' . ELEMENTOR_EXTRAS_ELEMENTOR_PRO_VERSION_REQUIRED . ' to avoid any issues. We\'ve deactivated Extras for now. Please update Elementor Pro and re-activate Extras.', 'elementor-extras' ) );
		}
	}
}
